...
type ProjectFilterFormData = ProjectFilterType;
...
function ProjectFilter() {
  ...
  const methods = useForm<ProjectFilterFormData>({
    defaultValues: {
      priority: "",
    },
  });
            ...
            <Grid container spacing={2} mb={3} mt={3}>
              <Grid item xs={12}>
                ...
                >
                  name="priority"
                  testId="priority"
                  label={t("admin-panel-Projects:filter.inputs.role.label")}
                  options={[
                    {
                      id: RoleEnum.ADMIN,
                    },
                  ...
                  renderOption={(option) =>
                    t(
                      `admin-panel-Projects:filter.inputs.role.options.${option.id}`
                    )
                  }
                  renderValue={(values) =>
                    values
                      .map((value) =>
                        t(
                          `admin-panel-Projects:filter.inputs.role.options.${value.id}`
                        )
                      )
                      .join(", ")
                  }
